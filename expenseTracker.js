...
`);
  ...
  if (bufferContent == undefined) { // first run procedure
    console.log(`2) Nothing in storage. Loading placeholder row to buffer...`);
    bufferContent = [`<p class = "expenseItem" ><span class = "date" ></span><button class="eraseExpense" >x</button><span class = "description" >Nothing posted yet   :-)</span><span class="amount" ></span ></p>`];
    console.log(`Done!!
  `);
  ...
  `);
  let rowsData = '';
  for (let pointerToRow = 0; pointerToRow < bufferContent.length; pointerToRow++) {
...
function postBalance() {
  ...
  //netBalance = Math.round(100 * netBalance()) / 100;
